...
function enableClick() {
	...
	if (!($.trim($taskText).length === 0)) {
		...
		if ((checkDuplicates($taskText, $list) == true) || ($('#listItems').children().length == 0)) {
			$('#listItems').append('<li class="curItem"><input type="checkbox" class = "item1" value= ""/>' + $taskText + '<img src="images/star-blank.png" height="20" width="20" style="display: inline-block"/></li>');
			$('#txt').val("");
